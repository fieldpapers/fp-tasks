...
export function convertPdfToGeoTIFF(payload, _callback) {
  ...
  return tmp.tmpName(function(err, filename) {
    ...
    gdalTranslate.on("exit", function(code) {
      if (code === 0) {
        return persister(fs.createReadStream(filename), (err, uri) => {
...
convertPdfToGeoTIFF.validate = function validate(payload) {

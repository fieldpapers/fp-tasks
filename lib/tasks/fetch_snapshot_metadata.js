...
module.exports = function fetchSnapshotMetadata(payload, _callback) {
  ...
  return async.waterfall([
    ...
    function(pageUrl, done) {
      return request.get({
        ...
        if (rsp.statusCode < 200 || rsp.statusCode >= 300) {
          return done(new Error(util.format("%s returned %d: %s", pageUrl, rsp.statusCode, rsp.body)));
        }
    ...
    if (err) {
      ...
      responsePayload.snapshot.page_url = pageUrl;
